$(document).ready(function () {
    ...
    const vm = new Vue({
        ...
        mounted() {
            ...
            $('#isOutSider').on('change', function () {
                ...
                if (!$('#isOutSider').is(':checked')) {
                    ...
                    $("#txtcgst").val(0);
                    $("#txtcgstPercentage").val(0);
                    vm.AddItem.IgstPercentage = (parseFloat(vm.AddItem.CgstPercentage) +
                        parseFloat(vm.AddItem.SgstPercentage)).toFixed(2);
                    vm.AddItem.Igst = (parseFloat(vm.AddItem.Cgst) + parseFloat(vm.AddItem.Sgst)).toFixed(2);
                    vm.AddItem.CgstPercentage = 0;
                    vm.AddItem.Cgst = 0;
                    vm.AddItem.SgstPercentage = 0;
                    vm.AddItem.Sgst = 0;
                }
            })
            //for customer
            ...
            $("#ddlProduct").on('change', function () {
                vm.AddItem.ProductId = $(this).val();
                vm.onChange(vm.AddItem.ProductId);
            })
        ...
        watch: {
            'AddItem.Discount'(newVal) {
                let taxvalue = (this.AddItem.Rate * parseInt(this.AddItem.Qty))
                    * (this.AddItem.Discount / 100);
                let beforeTaxvalue = ((parseInt(this.AddItem.Qty) * this.AddItem.Rate) - taxvalue);
                this.AddItem.BaseValue = (parseFloat(beforeTaxvalue)).toFixed(2);
                //  let BaseValue = (this.BillingItem.Rate + taxvalue).toFixed(2);
                //change amount with tax
                let cgstAmount = (beforeTaxvalue.toFixed(2) * (this.AddItem.CgstPercentage / 100));
                //sgst amount
                let sgstAmount = (beforeTaxvalue.toFixed(2) * (this.AddItem.SgstPercentage / 100));
                //igst amount
                let igstAmount = (beforeTaxvalue.toFixed(2) * (this.AddItem.IgstPercentage / 100));


                this.AddItem.Cgst = cgstAmount.toFixed(2);
                this.AddItem.Sgst = sgstAmount.toFixed(2);
                this.AddItem.Igst = igstAmount.toFixed(2);

                //Amount value
                this.AddItem.Amount = (parseFloat(this.AddItem.Cgst) +
                    parseFloat(this.AddItem.Sgst) +
                    parseFloat(this.AddItem.Igst) +
                    parseFloat(beforeTaxvalue.toFixed(2))).toFixed(2);
            },
            'EditItem.Discount'(newVal) {
                let taxvalue = (this.EditItem.Rate * parseInt(this.EditItem.Qty)) * (this.EditItem.Discount / 100);
                let beforeTaxvalue = ((parseInt(this.EditItem.Qty) * this.EditItem.Rate) - taxvalue);
                this.BillingItem.BaseValue = (parseFloat(beforeTaxvalue)).toFixed(2);
                //  let BaseValue = (this.BillingItem.Rate + taxvalue).toFixed(2);
                //change amount with tax
                let cgstAmount = (beforeTaxvalue.toFixed(2) * (this.EditItem.CgstPercentage / 100));
                //sgst amount
                let sgstAmount = (beforeTaxvalue.toFixed(2) * (this.EditItem.SgstPercentage / 100));
                //igst amount
                let igstAmount = (beforeTaxvalue.toFixed(2) * (this.EditItem.IgstPercentage / 100));


                this.EditItem.Cgst = cgstAmount.toFixed(2);
                this.EditItem.Sgst = sgstAmount.toFixed(2);
                this.EditItem.Igst = igstAmount.toFixed(2);

                //Amount value
                this.EditItem.Amount = (parseFloat(this.EditItem.Cgst) +
                    parseFloat(this.EditItem.Sgst) +
                    parseFloat(this.EditItem.Igst) +
                    parseFloat(beforeTaxvalue.toFixed(2))).toFixed(2);
            },
        },
        destroyed: function () {
        ...
        methods: {
            ...
            onSubmit: function () {
                ...
                const { BillingItems } = vm.BillingVm;
                ...
                let salesVm = {
                    //Id: vm.salesVm.id,
                    CustomerId: vm.BillingVm.CustomerId,
                    InvoiceDate: vm.BillingVm.InvoiceDate,
                    InvoiceNo: vm.BillingVm.InvoiceNo,
                    OtherCustomer: vm.BillingVm.OtherCustomer,
                    TotalBaseValue: vm.BillingVm.TotalBaseValue,
                    TotalCgst: vm.BillingVm.TotalCGSTAmount,
                    TotalAmount: vm.BillingVm.TotalAmount,
                    TotalSgst: vm.BillingVm.TotalSGSTAmount,
                    TotalIgst: vm.BillingVm.TotalIGSTAmount,
