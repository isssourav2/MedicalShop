    ...
    const vm = new Vue({
        ...
        mounted() {
            ...
            this.getVendor().then(v => {
                ...
                $(`#ddlVendors option[value=${vendorId}]`).attr('selected', 'selected');
               // $(`#ddlVendors option[text=${vendorName}]`)
                $('#ddlVendors').val(vendorId);
            })
            $("#ddlVendors").on('change', function () {
                vm.purchaseVm.vendorId = $(this).val();
            })
            //list convert to model of edit
            orderItems.map(dt => {
                const { id, baseid, poId, qty, productId, productName, package, mrp, rate, discount, taxValue, hsn, batch, cgst, cgstPercentage, sgst, sgstPercentage, igst, igstPercentage, amount } = dt;
                let orderitem = {};
                orderitem.Id = id;
                orderitem.Baseid = baseid;
                orderitem.PoId = poId;
                orderitem.Qty = qty;
                orderitem.ProductId = productId;
                orderitem.ProductName = productName;
                orderitem.Package = package;
                orderitem.Mrp = mrp;
                orderitem.Rate = rate;
                orderitem.Discount = discount;
                orderitem.TaxValue = taxValue;
                orderitem.Hsn = hsn;
                orderitem.Batch = batch;
                orderitem.Cgst = cgst;
                orderitem.CgstPercentage = cgstPercentage;
                orderitem.Sgst = sgst;
                orderitem.SgstPercentage = sgstPercentage;
                orderitem.Igst = igst;
                orderitem.IgstPercentage = igstPercentage;
                orderitem.Amount = amount;
                vm.purchaseOrderItems.push(orderitem);
            })
            //For Product
            ...
            this.getDoctor().then(v => {
                ...
                $(`#ddlDoctors option[value=${doctorId}]`).attr('selected', 'selected');
               // $(`#ddlDoctors option[text=${doctorName}]`)
                $('#ddlDoctors').val(doctorId);
            })
        ...
        methods: {
            ...
            onChange(event) {
                ...
                let orderitems = this.purchaseOrderItems.filter(function (obj) {
                    return obj.ProductId == parseInt(event);
                })
            ...
            onOrderItemSaved: function () {
                if (this.purchaseOrderItem.ProductId != 0) {
                   ...
                   // this.purchaseVm.PurchaseOrderItems = [];
                    ...
                    //this.purchaseVm.TotalAmount = (parseFloat(this.purchaseVm.TotalAmount) + parseFloat(this.purchaseTotalAmount)).toFixed(2);
                    ...
                    this.purchaseVm.TotalAmount = TotalAmount;
            ...
            onUpdateData: function () {
             ...
             //   this.purchaseTotalAmount = (parseFloat(this.purchaseVm.TotalAmount)+ parseFloat(this.purchaseOrderItem.Amount));
                //this.purchaseVm.TotalAmount = parseFloat(this.purchaseVm.TotalAmount)+ purchaseAmount.toFixed(2);
                //this.purchaseVm.TotalCGSTAmount = parseFloat(this.purchaseVm.TotalCGSTAmount)+ purchasecgstAmount.toFixed(2);
                //this.purchaseVm.TotalSGSTAmount = parseFloat(this.purchaseVm.TotalSGSTAmount)+ purchasesgstAmount.toFixed(2);
                //this.purchaseVm.TotalIGSTAmount = parseFloat(this.purchaseVm.TotalIGSTAmount) + purchaseigstAmount.toFixed(2);
